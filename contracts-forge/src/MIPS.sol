...
contract MIPS {
    ...
    uint32 public constant REG_OFFSET = 0xc0000000;
    uint32 public constant REG_ZERO = REG_OFFSET;
    uint32 public constant REG_LR = REG_OFFSET + 0x1f * 4;
    uint32 public constant REG_PC = REG_OFFSET + 0x20 * 4;
    uint32 public constant REG_HI = REG_OFFSET + 0x21 * 4;
    uint32 public constant REG_LO = REG_OFFSET + 0x22 * 4;
    uint32 public constant REG_HEAP = REG_OFFSET + 0x23 * 4;

    uint32 public constant HEAP_START = 0x20000000;
    ...
    function Steps(bytes32 stateHash, uint256 count) public returns (bytes32) {
    ...
    function stepPC(bytes32 stateHash, uint32 pc, uint32 nextPC) internal returns (bytes32) {
        ...
        if (opcode == 2 || opcode == 3) {
            stateHash = stepPC(stateHash, nextPC, SE(insn & 0x03FFFFFF, 26) << 2);
            if (opcode == 3) {
        ...
        if ((opcode >= 4 && opcode < 8) || opcode == 1) {
            ...
            if (shouldBranch) {
                return stepPC(stateHash, nextPC, pc + 4 + (SE(insn & 0xFFFF, 16) << 2));
            }
            // branch not taken
