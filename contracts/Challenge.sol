...
contract Challenge {
    ...
    returns (uint256)
  {
    ...
    bytes32 inputHash;
    {
      ...
      bytes32 coinbase  = bytes32(bytes20(Lib_RLPReader.readAddress(decodedHeader[2])));
  ...
  /// @notice If the search is finished (cf. `isSearching`), returns the state proposed by the
  ///         challenger for the single step of disagreement, otherwise reverts.
