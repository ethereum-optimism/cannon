...
contract Challenge {
    ...
    function initiateChallenge(
        ...
        require(
            mem.ReadMemory(finalSystemState, 0xC0000080) == 0x5EAD0000,
            "the final MIPS machine state is not stopped (PC != 0x5EAD0000)"
        );
        require(
            mem.ReadMemory(finalSystemState, 0x30000800) == 0x1337f00d,
            "the final state root has not been written a the predefined MIPS memory location"
        );
        require(
            mem.ReadBytes32(finalSystemState, 0x30000804) == assertionRoot,
    ...
    function isSearching(uint256 challengeId) public view returns (bool) {
        ChallengeData storage c = challenges[challengeId];
    ...
    ///         binary search.
    function getStepNumber(uint256 challengeId) public view returns (uint256) {
        ChallengeData storage c = challenges[challengeId];
        require(c.challenger != address(0), "invalid challenge");
        return (c.L + c.R) / 2;
    }
    ...
    function getProposedState(uint256 challengeId) public view returns (bytes32) {
        ChallengeData storage c = challenges[challengeId];
        require(c.challenger != address(0), "invalid challenge");
