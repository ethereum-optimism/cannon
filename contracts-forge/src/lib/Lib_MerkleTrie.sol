...
library Lib_MerkleTrie {
     ...
     */
    function update(bytes memory _key, bytes memory _value, bytes32 _root) internal returns (bytes32 _updatedRoot) {
    ...
    function getSingleNodeRootHash(bytes memory _key, bytes memory _value) internal returns (bytes32 _updatedRoot) {
        bytes memory dat = _makeLeafNode(Lib_BytesUtils.toNibbles(_key), _value).encoded;
    ...
    function _getNewPath(
        ...
        if (lastNodeType == NodeType.LeafNode) {
            ...
            if (
                _getSharedNibbleLength(_getNodeKey(lastNode), Lib_BytesUtils.slice(Lib_BytesUtils.toNibbles(_key), l))
                    == _getNodeKey(lastNode).length && keyRemainder.length == 0
            ) {
                matchLeaf = true;
            }
        }
     ...
     */
    function _getNodeID(Lib_RLPReader.RLPItem memory _node) private pure returns (bytes32 _nodeID, uint256 length) {
        bytes memory nodeID;
    ...
    function _getNodeValue(TrieNode memory _node) private pure returns (bytes memory _value) {
        ...
        (uint256 itemOffset, uint256 itemLength, Lib_RLPReader.RLPItemType itemType) = Lib_RLPReader._decodeLength(_in);
     ...
     */
    function _makeExtensionNode(bytes memory _key, bytes memory _value, bool isBytes)
        private
        pure
