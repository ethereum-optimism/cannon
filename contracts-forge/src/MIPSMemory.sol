...
contract MIPSMemory {
    ...
    function AddLargePreimageInit(uint256 offset) public {
        require(offset & 3 == 0, "offset must be 32-bit aligned");
    ...
    function AddLargePreimageUpdate(bytes calldata dat) public {
        ...
        int256 offset = int256(largePreimage[msg.sender].offset) - int256(largePreimage[msg.sender].len);
        if (offset >= 0 && offset < 136) {
            largePreimage[msg.sender].data = fbo(dat, uint256(offset));
        }
    ...
    function AddLargePreimageFinal(bytes calldata idat) public view returns (bytes32, uint32, uint32) {
        ...
        int256 offset = int256(largePreimage[msg.sender].offset) - int256(largePreimage[msg.sender].len);
        require(offset < int256(idat.length), "offset must be less than length");
        Lib_Keccak256.CTX memory c;
        c.A = largePreimageState[msg.sender];

        bytes memory dat = new bytes(136);
        for (uint256 i = 0; i < idat.length; i++) {
            dat[i] = idat[i];
        }
        uint256 len = largePreimage[msg.sender].len + idat.length;
        uint32 data = largePreimage[msg.sender].data;
        if (offset >= 0) {
            data = fbo(dat, uint256(offset));
        }
